...
$$
Отличие полученного значения от экспериментального главным образом объясняется тем, что мы пренебрегли массой ядра. Если её учесть, то получим: $R = \frac{R_\text{теор}}{1+m/M} = 109677,6\ \text{см}^{-1}$, что практически совпадает с экспериментальным значением.
