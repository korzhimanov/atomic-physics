...
Серия при $n_1 = 2$ называется серией Бальмера. Она была открыта первой в 1885 году:
...
Для каждой серии имеется также граница серии --- значение длины волны, к которой стремится серия при $n\to\infty$. Например, для серии Бальмера $\lambda_\infty = 364,47$ нм.
...
Расмотрим вслед за Бором приложение его теории к так называемым водородоподбным атомам, то есть атомам, на орбите которых имеется только один электрон. Простейшим примером таких атомов является собственно водород, а также его изотопы. Другие примеры --- это ионы He<sup>+1</sup>, Li<sup>+2</sup> и т. д.
...
$$
R_{теор} = \frac{me^4}{64\pi^3\varepsilon_0^2\hbar^3c} = \frac{me^4}{8\varepsilon_0^2h^3c} = 109737\ см^{-1}
$$
Отличие полученного значения от экспериментального главным образом объясняется тем, что мы пренебрегли массой ядра. Если её учесть, то получим: $R = \frac{R_{теор}}{1+m/M} = 109677,6$ см$^{-1}$, что практически совпадает с экспериментальным значением.
