...
В то же время в силу равенства нулю попарных коммутаторов операторов $\hat {L^2}$, $\hat L_z$, $\hat L_{1z}$, $\hat L_{2z}$ существует набор состояний $\Psi_{l_1,l_2,l,m}$, в которых определены одновременно и собственные значения операторов $\hat{L_1^2}$, $\hat{L_2^2}$, и собственные значения операторов $\hat {L^2}$, $\hat L_z$, характеризуемые числами $l, m$. При этом поскольку этот набор состояний также является полным, то их число также должно быть равно $(2l_1+1)(2l_2+2)$. Это позволяет найти связь между числами $l, m$ и $l_1,m_1,l_2,m_2$.
